...
std::vector<float> terrain::getTerrainData(GLuint& terrain_vbo, GLuint& terrain_vao) {
    std::vector<float> interleavedData = LoadGLBVerticesNormals("../../src/terrain/models/brain_coral.glb");

    // Generate and bind VBO
    glGenBuffers(1, &terrain_vbo);
    glBindBuffer(GL_ARRAY_BUFFER, terrain_vao);
